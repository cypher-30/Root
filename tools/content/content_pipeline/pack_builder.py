...
from .limits import (
    ...
    AUDIO_FILENAME_PREFIX, manifest_key_for,
)
...
class AssetInput:
    ...
    caller/source filename: the parent transport's convention is that
    downloaded media is always named 'audio-<assetId><ext>', so it is
    derived deterministically from asset_id + mime here. codec/duration are
    used internally for media probing (media.py) but codec is excluded from
    the wire ManifestAsset shape."""
    asset_id: str
    source_path: Path
    mime: str
...
def _relative_key_for_asset(asset_id: str, mime: str) -> str:
    _validate_id("Asset", asset_id)
    ext = _MIME_TO_EXT.get(mime)
    if ext is None:
        raise PackBuildError("Unsupported mime type for asset " + asset_id + ": " + mime)
    key = "assets/" + AUDIO_FILENAME_PREFIX + asset_id + ext
...
def build_manifest(
    ...
    for a in assets:
        ...
        relative_key = _relative_key_for_asset(a.asset_id, a.mime)
